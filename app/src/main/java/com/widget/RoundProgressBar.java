...
public class RoundProgressBar extends View {
    ...
    private RectF oval = new RectF(0, 0, 0, 0);//弧形区域
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (textIsDisplayable && style == STROKE) {
            canvas.drawText(percent + "%", centre - textWidth / 2, baseline, paint); // 画出进度百分比
        }
        ...
        switch (style) {
            case STROKE: {
                ...
                canvas.drawArc(oval, -90, 360 * progress / max, false, paint); // 根据进度画圆弧,第三个参数是扫过的角度，不是终点角度
                break;
            }
            case FILL: {
